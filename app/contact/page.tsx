    ...
    <div className="min-h-screen bg-[#060606] pt-28 pb-20 px-6 md:px-12">
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="mb-16">
          <span className="text-[10px] tracking-[0.5em] text-[#d5001c] uppercase font-bold block mb-4">Fale Conosco</span>
          <h1 className="font-display text-5xl md:text-7xl italic text-white mb-6" style={{ letterSpacing: "-0.02em" }}>Consultoria Exclusiva</h1>
          <p className="text-white/40 text-sm leading-relaxed max-w-lg tracking-wide">
            Agende uma visita privada ou consulta com nossos embaixadores de marca. Sua jornada começa aqui.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-20">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
            <div className="space-y-8 mb-12">
              {[
                { label: "Localização", value: "Stuttgart, Alemanha" },
                { label: "E-mail", value: "exclusive@porsche.com" },
                { label: "Telefone", value: "+49 711 911 0" },
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3 }}>
            {sent ? (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-20">
                ...
                <h3 className="font-display text-2xl italic text-white mb-3">Mensagem Recebida</h3>
                <p className="text-white/40 text-sm tracking-wide">Nossa equipe entrará em contato em até 24 horas.</p>
              </motion.div>
            ) : (
              <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="space-y-6">
                {[
                  { placeholder: "Nome Completo", type: "text" },
                  { placeholder: "Endereço de E-mail", type: "email" },
                  { placeholder: "Número de Telefone", type: "tel" },
                ].map((f) => (
                  <input
                    key={f.placeholder}
                    type={f.type}
                ...
                <select className="w-full bg-transparent border-0 border-b border-white/10 text-white/40 text-xs tracking-widest py-4 outline-none focus:border-[#d5001c] transition-colors uppercase">
                  <option value="">Selecione o Modelo de Interesse</option>
